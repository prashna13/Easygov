...
- [x] **Flatten M3 redesign (Android)**: every screen converted from colored header bands to the flat design system — `bg_screen` + `bg_panel` (16dp radius) + `bg_icon_soft` wells, edge-to-edge transparent top bars with back buttons, `Widget.Easygov.Card`/`Button`/`OutlinedButton`/`SegmentButton`, pill-height soft active indicator on the bottom nav, Material `Chip`-style buttons for CTA strips; screens touched: Dashboard, Chat, Documents, Profile, Login, Register, Onboarding, ServiceDetail, ApplicationProgress, NearbyOffices; all strings/ids preserved; APK rebuilt and verified (build green, no crashes, correct nav geometry on emulator)
...
| 2026-08-19 | **Flatten M3 redesign (Android, panel A: styles + navigation)**: fixed the bottom bar to the new flat design system — material 1.12.0 ships `Widget.Material3.BottomNavigationView`/`.ActiveIndicator` (no `NavigationBarView`/`TopAppBar`/`SegmentedButton` variants, and `NavigationBarView` lacks the `(Context, AttributeSet)` ctor so XML inflation crashes); rewrote `Widget.Easygov.NavigationBar`(+`ActiveIndicator`) onto the real M3 parents (standalone AAPT2 styles need explicit parents, else dotted names inherit a missing `Widget.Easygov` prefix), switched `activity_main.xml` to `BottomNavigationView`; fixed false attr names (`itemLabelVisibilityMode`→`labelVisibilityMode`, `chipTextColor`→`android:textColor`); created `bg_accent_filled_round` (theme-primary, 12dp) + `ic_add`; added missing strings (`dash_welcome_prefix`, `search`, `search_services`, `dash_view_requirements`); removed `setLineSpacingExtra` unresolved call in ChatFragment. Build green; APK installed on emulator — no crashes, all tabs render (initial crash `NoSuchMethodException NavigationBarView.<init>` + full-screen nav bar `0,0-1080,2400` both fixed) |
| 2026-08-20 | **Office catalog refresh (backend)**: merged the user's curated valley-office dataset into `app/office_seed_data.py` — **34 → 50 offices**. Fixed coordinate/contact mismatches on 3 already-seeded DAOs (Kathmandu → Babarmahal 27.6915,85.3240; Lalitpur → Manbhawan 27.6685,85.3180; Bhaktapur → Byasi 27.6728,85.4285) and **added 16 new offices** (valley TMO driving-license offices Ekantakuna/Chabahil/Thulobharyang/Radhe Radhe; Area Administration Sankhu + Gotikhel; Department of Passports (DoP) + 3 DAO Passport sections; DoNIDCR HQ + Rastriya Sabha Griha + Narayanhiti NID centers + 3 DAO NID biometric stations). Deactivated 3 legacy near-duplicates (`is_active=False`, persisted in the seed file so re-seeds keep them hidden): Department of Transport Management, Department of Passports (Rahadani Bhaban), DoNIDCR Central Office. **47 active offices**; all 25 office tests pass |
